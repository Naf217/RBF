--// Chloe X Style UI (Bigger Version) by Majid
...
infoText.Text = "Versi: ChloeX UI\nStatus: Aktif\n\nIni adalah panel info dengan ukuran besar seperti Chloe."
...
local sLabel = Instance.new("TextLabel", serverFrame)
sLabel.Text = "Rejoin Server"
sLabel.Size = UDim2.new(0,240,0,32)
sLabel.Position = UDim2.new(0,10,0,10)
...
toggle.Position = UDim2.new(0,265,0,10)
...
toggle.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 then
        on = not on
        if on then
            tween:Create(knob,TweenInfo.new(0.2),{Position = UDim2.new(1,-25,0,1)}):Play()
            knob.BackgroundColor3 = Color3.fromRGB(80,255,80)
            game:GetService("TeleportService"):Teleport(game.PlaceId,game.Players.LocalPlayer)
        else
            tween:Create(knob,TweenInfo.new(0.2),{Position = UDim2.new(0,1,0,1)}):Play()
            knob.BackgroundColor3 = Color3.fromRGB(210,210,210)
        end
    end
end)
